...
const library: LibraryAvailability = {
  milestone: (key, query) =>
    query.language === 'ru' || key !== 'diagnostic'
      ? { key, itemId: `item-${key}`, title: `${key} ${query.category ?? 'universal'} ${query.gender ?? 'any'}` }
      : null,
...
function analysis(patch: Partial<Analysis> = {}): Analysis {
  return {
    ...
    interest: 1,
    mood: 'calm',
    answerPoints: [{ id: 'p1', text: 'спросил, где ты живёшь', kind: 'question', topic: 'practitioner', skip: false }],
    ...patch,
  };
}
...
const said = (kind: SaidEntry['kind'], key: string, messageId: number | null, minutes: number): SaidEntry => ({
  kind,
  key,
  messageId,
...
const withBirth = { birthDate: { value: '04.01.1999', confidence: 1 }, birthPlace: { value: 'Москва', confidence: 1 } };

describe('план: передача менеджеру', () => {
  it.each([
    ['медиа', input({ messages: [{ id: 1, text: '', mediaKind: 'voice', sentAt: T0 }] }), 'media'],
    ['риск', input({ analysis: analysis({ risk: ['asks_if_bot'] }) }), 'risk'],
    ['ответ после цен', input({ stage: 'prices' }), 'reply_after_prices'],
    ['язык без материалов', input({ memory: memory({ card: { language: { value: 'other', confidence: 1 } } }) }), 'no_language_materials'],
...
describe('план: знакомство', () => {
  it('первый ход: поздороваться, ответить, попросить данные', () => {
    ...
    expect(plan.answer.map((point) => point.text)).toEqual(['спросил, где ты живёшь']);
    ...
    expect(plan.constraints.doNotMention).toContain('цены и суммы своими словами');
  ...
  it('первый ход клиента, который сразу всё рассказал: данные ещё не просили — просим, ссылки в том же ходе не шлём', () => {
    const plan = buildPlan(input({ analysis: analysis({ intents: ['shares_story'], interest: 2 }) }));
    expect(plan.nudge).toBe('ask_birth_data');
    expect(plan.milestone).toBeNull();
  });

  it('данные есть, запроса нет: спросить о запросе, не больше двух раз', () => {
    const base = input({ memory: memory({ card: withBirth, said: [said('nudge', 'ask_birth_data', 2, 30)] }), state: { turnsWithoutNudge: 0, remindersSent: 0, turnsInStage: 1 } });
    expect(buildPlan(base).nudge).toBe('ask_request');
    const twice = input({
      ...base,
      memory: memory({ card: withBirth, said: [said('nudge', 'ask_birth_data', 2, 30), said('nudge', 'ask_request', 4, 20), said('nudge', 'ask_request', 6, 10)] }),
  ...
  it('вопрос о цене до предложения — отложенный ответ, не отказ', () => {
    const plan = buildPlan(
      input({
        analysis: analysis({
          intents: ['asks_price'],
          answerPoints: [{ id: 'p1', text: 'спросила, сколько стоит работа', kind: 'question', topic: 'price', skip: false }],
        }),
      }),
    );
    ...
    const plan = buildPlan(
      input({
        analysis: analysis({
          ...
          answerPoints: [
            { id: 'p1', text: 'спросила, сколько стоит работа', kind: 'question', topic: 'price', skip: false },
            { id: 'p2', text: 'спросила, сколько ждать диагностику', kind: 'question', topic: 'diagnostic', skip: false },
            { id: 'p3', text: 'спросила, где ты живёшь', kind: 'question', topic: 'practitioner', skip: false },
          ],
        }),
      }),
    );
  ...
  it('веха «ссылки» сама отвечает на вопрос о диагностике — формула не нужна', () => {
    const asks = analysis({
      ...
      answerPoints: [{ id: 'p1', text: 'спросила, когда будет расклад', kind: 'question', topic: 'diagnostic', skip: false }],
    });
    const plan = buildPlan(
      input({
        analysis: asks,
        memory: memory({ card: withBirth, said: [said('nudge', 'ask_birth_data', 2, 10)] }),
  ...
  it('ссылки: запрос рассказан, данные есть, ход не первый', () => {
    const plan = buildPlan(
      input({
        ...
        memory: memory({ card: { ...withBirth, category: { value: 'relationships.breakup', confidence: 0.9 } }, said: [said('nudge', 'ask_birth_data', 2, 5)] }),
        state: { turnsWithoutNudge: 0, remindersSent: 0, turnsInStage: 1 },
      }),
    );
    expect(plan.milestone?.key).toBe('links');
...
describe('план: ожидание диагностики', () => {
  ...
  it('клиент пишет во время ожидания: отвечаем, вехи нет, содержание не раскрываем', () => {
    const plan = buildPlan(input({ stage: 'links', memory: memory({ card: withBirth, said: links }) }));
    expect(plan.milestone).toBeNull();
    expect(plan.nudge).toBeNull();
    expect(plan.goal).toContain('Поддерживай ожидание');
    expect(plan.constraints.doNotMention).toContain('содержание и выводы диагностики');
  });
  ...
  it('«ну что там?» после минимума — диагностика в этом же ходе', () => {
    const plan = buildPlan(
      input({
        stage: 'links',
        analysis: analysis({ intents: ['asks_diagnostic_status'], card: { gender: { value: 'f', confidence: 0.9 }, category: { value: 'relationships.breakup', confidence: 0.9 } } }),
        memory: memory({ card: { ...withBirth, gender: { value: 'f', confidence: 0.9 }, category: { value: 'relationships.breakup', confidence: 0.9 } }, said: links }),
      }),
    );
    expect(plan.milestone).toEqual({ key: 'diagnostic', itemId: 'item-diagnostic', title: 'diagnostic relationships.breakup f' });
  });
  ...
  it('«ну что там?» слишком рано — ждём', () => {
    const plan = buildPlan(
      input({
        ...
        memory: memory({ card: withBirth, said: [said('milestone', 'links', 5, 10)] }),
      }),
    );
    expect(plan.milestone).toBeNull();
  });
  ...
  it('таймер диагностики по расписанию; пол не уверен — универсальная', () => {
    const plan = buildPlan(
      input({
        ...
        analysis: null,
        stage: 'links',
        memory: memory({ card: { ...withBirth, gender: { value: 'f', confidence: 0.5 } }, said: links }),
      }),
    );
  ...
  it('диагностика созрела, а данных нет и напоминания не было — сначала напоминание', () => {
    const plan = buildPlan(
      input({ trigger: 'schedule', job: { id: 'j1', kind: 'diagnostic' }, messages: [], analysis: null, stage: 'links', memory: memory({ said: links }) }),
    );
    expect(plan.milestone).toBeNull();
  ...
  it('данных нет, но напоминание уже было — универсальная диагностика', () => {
    const plan = buildPlan(
      input({
        ...
        analysis: null,
        stage: 'links',
        memory: memory({ said: [...links, said('nudge', 'birth_data_reminder', 7, 20)] }),
      }),
    );
  ...
  it('нет диагностики на языке клиента — менеджер', () => {
    const plan = buildPlan(
      input({
        ...
        analysis: null,
        stage: 'links',
        memory: memory({ card: { ...withBirth, language: { value: 'en', confidence: 1 } }, said: links }),
      }),
    );
...
describe('план: после диагностики и предложения', () => {
  const diagnosticSaid = [said('milestone', 'links', 5, 200), said('milestone', 'diagnostic', 9, 100)];
  ...
  it('диагностика не прочитана — предложения нет, даже по расписанию, и ход пустой', () => {
    const plan = buildPlan(
      input({ trigger: 'schedule', job: { id: 'j', kind: 'offer' }, messages: [], analysis: null, stage: 'diagnostic', memory: memory({ said: diagnosticSaid }), history: [out(9, 100, false)] }),
    );
    expect(plan.milestone).toBeNull();
  ...
  it('устаревшие задания пропускаются без текста', () => {
    const schedule = (kind: 'diagnostic' | 'birth_data_reminder' | 'return_question', patch: Partial<PlanInput>) =>
      buildPlan(input({ trigger: 'schedule', job: { id: 'j', kind }, messages: [], analysis: null, ...patch }));
    // Диагностика уже ушла (например, в ходе клиента по срочному заданию).
    expect(schedule('diagnostic', { stage: 'diagnostic', memory: memory({ said: diagnosticSaid }), history: [out(9, 100, true)] }).idle).not.toBeNull();
    // Данные пришли раньше напоминания.
    expect(schedule('birth_data_reminder', { memory: memory({ card: withBirth }) }).idle).not.toBeNull();
    expect(schedule('birth_data_reminder', {}).nudge).toBe('birth_data_reminder');
    // Этап сменился — вопрос-отклик после диагностики уже не к месту.
    expect(schedule('return_question', { stage: 'offer', memory: memory({ said: [...diagnosticSaid, said('milestone', 'offer', 12, 30)] }) }).idle).not.toBeNull();
  });

  it('таймер диагностики раньше данных: сначала напоминание', () => {
    const plan = buildPlan(
      input({ trigger: 'schedule', job: { id: 'j', kind: 'diagnostic' }, messages: [], analysis: null, stage: 'links', memory: memory({ said: [said('milestone', 'links', 5, 120)] }) }),
    );
    expect(plan.milestone).toBeNull();
    expect(plan.nudge).toBe('birth_data_reminder');
  ...
  it('слабый отклик — вопрос-отклик, потом без повтора', () => {
    const base = input({ stage: 'diagnostic', analysis: analysis({ intents: ['silent_ack'], interest: 1 }), memory: memory({ said: diagnosticSaid }), history: [out(9, 100, true)] });
    expect(buildPlan(base).nudge).toBe('ask_feedback');
    const again = input({ ...base, memory: memory({ said: [...diagnosticSaid, said('nudge', 'ask_feedback', 11, 30)] }) });
    expect(buildPlan(again).nudge).toBeNull();
    expect(buildPlan(again).constraints.doNotRepeat).toContain('вопрос: ask_feedback');
  });

  it('возражение: первый подход, при повторе — следующий', () => {
    const base = input({ stage: 'offer', analysis: analysis({ intents: ['objects'], objection: 'expensive' }), memory: memory({ said: [...diagnosticSaid, said('milestone', 'offer', 12, 30)] }), history: [out(12, 30, true)] });
    expect(buildPlan(base).objection).toEqual({ category: 'expensive', approach: 0 });
    const repeated = input({ ...base, memory: memory({ said: [...diagnosticSaid, said('milestone', 'offer', 12, 30), said('argument', 'expensive:0', 13, 10)] }) });
  ...
  it('спросил цену после прочитанного предложения — цены; до прочтения — нет', () => {
    ...
    const asksPrice = analysis({
      intents: ['asks_price'],
      answerPoints: [{ id: 'p1', text: 'спросил, сколько стоит', kind: 'question', topic: 'price', skip: false }],
    });
    const priced = buildPlan(input({ stage: 'offer', analysis: asksPrice, memory: memory({ said: offerSaid }), history: [out(12, 30, true)] }));
    ...
    const unread = buildPlan(input({ stage: 'offer', analysis: asksPrice, memory: memory({ said: offerSaid }), history: [out(12, 30, false)] }));
  ...
  it('ступени молчания считаются напоминаниями, вехи — нет', () => {
    const nudge = buildPlan(input({ trigger: 'schedule', job: { id: 'j', kind: 'return_question' }, messages: [], analysis: null, stage: 'diagnostic', memory: memory({ said: diagnosticSaid }), history: [out(9, 100, true)] }));
    expect(nudge.nudge).toBe('ask_feedback');
    expect(nudge.reminders).toBe(1);
    const prices = buildPlan(input({ trigger: 'schedule', job: { id: 'j', kind: 'prices' }, messages: [], analysis: null, stage: 'offer', memory: memory({ said: [...diagnosticSaid, said('milestone', 'offer', 12, 30)] }), history: [out(12, 30, true)] }));
