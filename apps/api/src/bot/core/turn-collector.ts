...
export class TurnCollector {
  ...
  typing(chatId: string, now: Date, extendMs: number): void {
    const current = this.pending.get(chatId);
    if (!current) return;
    current.deadline = Math.min(
      Math.max(current.deadline, now.getTime() + extendMs),
  ...
  take(
    chatId: string,
  ): { messages: IncomingMessage[]; generationSeq: number } | null {
    const pending = this.pending.get(chatId);
    if (!pending) return null;
    this.pending.delete(chatId);
    return {
      messages: pending.messages,
      generationSeq: this.generation.get(chatId) ?? 0,
    };
  }
