...
import { argumentsUsed, clientLanguage, hasBirthData, knownCategory, knownGender, nudgesSaid } from './memory.js';
import type {
  Analysis,
  AnswerTopic,
  HistoryMessage,
...
const REMINDER_JOBS: readonly JobKind[] = ['return_question', 'offer_nudge', 'unread_reminder'];
 ...
 */
const TOPIC_HOLDS: Partial<Record<AnswerTopic, { opensAt: Stage; hold: string; coveredBy?: Milestone }>> = {
  price: {
    opensAt: 'prices',
...
const ENGAGED_INTENTS = new Set(['shares_story', 'asks_about_practitioner', 'asks_practice', 'doubts', 'objects']);
...
function minutesBetween(from: Date | null, to: Date): number {
  return from ? (to.getTime() - from.getTime()) / 60_000 : Number.POSITIVE_INFINITY;
}
...
export function buildPlan(input: PlanInput): Plan {
  ...
  const plan: Plan = {
    ...
    nudge: null,
    objection: null,
    constraints: { doNotRepeat: [], doNotMention: [], language, maxParts: 3, hook: true },
    goal: '',
    reminders: 0,
    idle: null,
  };
  ...
  if (analysis && analysis.risk.length > 0) return handoff(plan, 'risk', analysis.risk.join(', '));
  if (stage === 'prices' && trigger === 'client') return handoff(plan, 'reply_after_prices', 'клиент ответил после цен');
  if (!input.library.supportsLanguage(language)) {
    return handoff(plan, 'no_language_materials', `нет материалов на языке «${language}»`);
  }

  // 2. Веха (ответы клиенту идут раньше неё в тексте, но какие темы можно раскрыть, зависит от вехи).
  const hasRequest = knownCategory(memory.card) !== null || intents.has('shares_story') || memory.facts.some((fact) => fact.kind === 'situation');
  ...
  const due = (kind: JobKind) => input.dueJobs.includes(kind) || job?.kind === kind;

  const wantMilestone = (key: Milestone): boolean => {
    switch (key) {
      case 'links': {
        // Данные рождения хотя бы раз просили: «займусь диагностикой» без них звучит странно.
        const askedBirth = hasBirthData(memory.card) || nudgesSaid(said, 'ask_birth_data') > 0;
        return stage === 'intake' && trigger === 'client' && askedBirth && (hasRequest || state.turnsInStage >= 2);
      }
      case 'diagnostic': {
        if (stage !== 'links') return false;
        const elapsed = minutesBetween(milestoneAt(said, 'links'), input.now);
        const asked = intents.has('asks_diagnostic_status') && elapsed >= timings.diagnosticDelayMin.min;
        return due('diagnostic') || asked;
      }
      case 'offer': {
        if (stage !== 'diagnostic' || !isRead(input.history, milestoneMessageId(said, 'diagnostic'))) return false;
        if (due('offer')) return true;
        return Boolean(analysis) && (analysis!.interest >= 2 || intents.has('asks_practice') || intents.has('ready'));
      }
      case 'prices': {
        if (stage !== 'offer' || !isRead(input.history, milestoneMessageId(said, 'offer'))) return false;
        return due('prices') || intents.has('asks_price') || intents.has('ready');
      }
      default:
        return false;
    }
  ...
  const target: Milestone | null = jobMilestone ?? (['links', 'diagnostic', 'offer', 'prices'] as Milestone[]).find(wantMilestone) ?? null;
  let diagnosticBlocked = false;
  const wanted = target !== null && wantMilestone(target);
  if (wanted && target === 'diagnostic' && !hasBirthData(memory.card) && nudgesSaid(said, 'birth_data_reminder') === 0) {
    ...
    if (item) {
      ...
      return handoff(plan, 'no_language_materials', `нет диагностики для «${category ?? 'универсальная'}», ${gender ?? 'любой пол'}, ${language}`);
    }
  }
  ...
  plan.answer = (analysis?.answerPoints ?? [])
    ...
    .map<PlannedAnswer>((point) => {
      ...
      const covered = rule?.coveredBy !== undefined && plan.milestone?.key === rule.coveredBy;
      const held = rule && before(reached, rule.opensAt) && !covered;
      return { ...point, hold: held ? rule.hold : null };
    });
  const priceHeld = plan.answer.some((point) => point.topic === 'price' && point.hold);
  ...
  if (analysis?.objection) {
    plan.objection = { category: analysis.objection, approach: argumentsUsed(said, analysis.objection) };
  }

  // 6. Ограничения.
  plan.constraints.doNotRepeat = [
    ...new Set(said.filter((entry) => entry.kind === 'nudge').map((entry) => `вопрос: ${entry.key}`)),
    ...said.filter((entry) => entry.kind === 'argument').map((entry) => `аргумент: ${entry.key}`),
  ];
  ...
  if (before(stage, 'diagnostic')) plan.constraints.doNotMention.push('подробное описание практик и услуг');
...
function handoff(plan: Plan, reason: Plan['handoff'] extends infer H ? (H extends { reason: infer R } ? R : never) : never, detail: string): Plan {
 ...
 */
function scheduledNudge(kind: JobKind, stage: Stage, memory: Memory): Nudge | null {
...
function clientNudge(
  ...
  const hasRequest = knownCategory(memory.card) !== null || memory.facts.some((fact) => fact.kind === 'situation');
  let candidate: Nudge | null = null;
  switch (stage) {
    case 'intake':
      if (!hasBirthData(memory.card) && nudgesSaid(said, 'ask_birth_data') === 0) candidate = 'ask_birth_data';
      else if (!hasRequest && nudgesSaid(said, 'ask_request') < 2) candidate = 'ask_request';
      break;
    case 'links':
      candidate = null;
      break;
    case 'diagnostic':
      ...
      break;
    case 'offer':
      if (nudgesSaid(said, 'ask_offer_questions') === 0) candidate = 'ask_offer_questions';
      break;
    default:
      candidate = null;
  }
  ...
  if (engaged && !firstTurn && state.turnsWithoutNudge < timings.maxTurnsWithoutNudge) return 'skip';
  return candidate;
}

const NUDGE_TEXT: Record<Nudge, string> = {
  ask_birth_data: 'попроси дату и место рождения, привязав к разговору',
  birth_data_reminder: 'мягко напомни, что для диагностики нужны дата и место рождения; если не пришлёт, сделаешь общий анализ',
  ask_request: 'спроси, что беспокоит и на какую сферу смотреть',
  ask_feedback: 'спроси, что из диагностики откликнулось и что хотелось бы изменить',
...
function describeGoal(plan: Plan, input: PlanInput, stage: Stage): string {
  ...
  if (plan.answer.length > 0) {
    ...
    lines.push(`Ответить (в этом порядке, раньше всего остального): ${items.join('; ')}.`);
  } else if (input.trigger === 'client') {
    lines.push('Ответить: отреагируй на сообщение клиента по существу.');
  }
  if (stage === 'intake' && input.state.turnsInStage === 0 && input.trigger === 'client') {
  ...
  if (plan.milestone) {
    ...
    lines.push('Шаг воронки: пропустить, клиент увлечён разговором. Закончи вопросом, который продолжает его тему.');
  } else if (plan.nudge) {
    lines.push(`Шаг воронки (один, после ответов, вплетённый в разговор): ${NUDGE_TEXT[plan.nudge]}.`);
  } else if (stage === 'links') {
    lines.push('Шаг воронки: нет. Поддерживай ожидание — ты сейчас смотришь карту клиента; содержание диагностики не раскрывай.');
  } else {
    lines.push('Шаг воронки: нет. Поддержи разговор и закончи вопросом или приглашением.');
  }
  if (plan.objection) {
    lines.push(
  ...
  if (plan.constraints.doNotRepeat.length > 0) lines.push(`Не повторять: ${plan.constraints.doNotRepeat.join(', ')}.`);
  if (plan.constraints.doNotMention.length > 0) lines.push(`Не упоминать: ${plan.constraints.doNotMention.join(', ')}.`);
  return lines.join('\n');
}
