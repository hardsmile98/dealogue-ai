...
function normalize(text: string): string {
  ...
  for (const char of text.toLowerCase()) {
    if (char.toLowerCase() !== char.toUpperCase() || (char >= '0' && char <= '9')) result += char;
  }
  return result;
}
 ...
 */
export function findMilestones(history: readonly HistoryMessage[], bodies: readonly MilestoneBody[]): FoundMilestone[] {
  const patterns = bodies
    .map((body) => ({ key: body.key, prefix: normalize(body.text).slice(0, BODY_PREFIX) }))
  ...
  for (const message of history) {
    ...
    if (match) found.set(match.key, { key: match.key, messageId: message.id, at: message.sentAt });
  }
 ...
 */
export function lastAnsweredIncoming(history: readonly HistoryMessage[]): number | null {
  let lastOut = -1;
