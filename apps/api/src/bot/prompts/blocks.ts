...
export function personaBlock(persona: Persona): string {
  const lines = [
    ...
    persona.bio ? `О себе: ${persona.bio}` : 'О себе: биография не заполнена — о своей жизни говори общо, без выдуманных фактов.',
  ];
  if (persona.links.length > 0) {
    lines.push(`Страницы: ${persona.links.map((link) => `${link.title} ${link.url}`).join('; ')}.`);
  }
  return lines.join('\n');
}
...
export function memoryBlock(memory: Memory): string {
  const card: string[] = [];
  const field = (label: string, value: string | undefined, confidence: number | undefined) =>
    value ? card.push(`${label}: ${value}${confidence !== undefined && confidence < 0.8 ? ' (не точно)' : ''}`) : undefined;
  field('Имя', memory.card.name?.value, memory.card.name?.confidence);
  field('Пол', memory.card.gender?.value === 'f' ? 'женщина' : memory.card.gender?.value === 'm' ? 'мужчина' : undefined, memory.card.gender?.confidence);
  field('Дата рождения', memory.card.birthDate?.value, memory.card.birthDate?.confidence);
  field('Место рождения', memory.card.birthPlace?.value, memory.card.birthPlace?.confidence);
  const category = REQUEST_CATEGORIES.find((item) => item.key === memory.card.category?.value);
  ...
  const lines = [`Карточка: ${card.length > 0 ? card.join('; ') : 'пока ничего не известно'}.`];
  if (memory.facts.length > 0) {
    lines.push('Факты о клиенте (свежие первыми):');
    for (const fact of memory.facts) lines.push(`- ${fact.text}${fact.confidence < 0.8 ? ' (не точно)' : ''}`);
  }
  lines.push(`Резюме разговора: ${memory.summary || 'разговор только начался'}.`);
  return lines.join('\n');
}

export function historyBlock(lines: readonly HistoryLine[]): string {
  if (lines.length === 0) return 'Переписки ещё не было.';
  return lines.map((line) => `${line.role === 'client' ? 'Клиент' : 'Ты'}: ${line.text}`).join('\n');
}
...
export function turnBlock(messages: readonly IncomingMessage[]): string {
  ...
  return messages.map((message) => (message.mediaKind ? `[${message.mediaKind}] ${message.text}`.trim() : message.text)).join('\n');
}
...
export function blockPreview(block: string): string {
  ...
  return text.length > BLOCK_PREVIEW_LENGTH ? `${text.slice(0, BLOCK_PREVIEW_LENGTH)}…` : text;
}
...
export function aboutBlock(samples: readonly LibrarySample[]): string {
  if (samples.length === 0) return 'Раздел «о себе и о работе» пуст: на вопросы о практике отвечай общо, без конкретных цифр, сроков и фактов.';
  return samples.map((sample) => `Вопрос: ${sample.title}\nОтвет: ${sample.text}`).join('\n\n');
}

export function examplesBlock(examples: readonly ExampleSample[]): string {
  if (examples.length === 0) return '';
  return examples
    .map((example) => `Ситуация: ${example.situation}\nКлиент: ${example.client}\nТы: ${example.practitioner}`)
    .join('\n\n');
}

export function categoriesBlock(): string {
  return REQUEST_CATEGORIES.map((category) => `${category.key} — ${category.title}`).join('\n');
}
