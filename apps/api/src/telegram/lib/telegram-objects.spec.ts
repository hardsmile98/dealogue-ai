...
describe('собеседник', () => {
  it('имя: «Имя Фамилия», иначе @username, иначе телефон', () => {
    expect(displayNameOf(user({ firstName: 'Анна', lastName: 'Ли' }))).toBe(
      'Анна Ли',
    );
  ...
  it('из диалогов берутся только личные чаты с людьми', () => {
    ...
    expect(
      privateUserOf({ isUser: true, entity: user({ bot: true }) }),
    ).toBeNull();
    expect(
      privateUserOf({ isUser: true, entity: { className: 'Channel' } }),
    ).toBeNull();
  });
});
...
describe('сообщение', () => {
  it('различает виды вложений', () => {
    ...
    expect(
      mediaKindOf({ className: 'MessageMediaWebPage' } as Api.TypeMessageMedia),
    ).toBeNull();
    expect(
      mediaKindOf({ className: 'MessageMediaPhoto' } as Api.TypeMessageMedia),
    ).toBe('photo');
    expect(
      mediaKindOf(
        documentMedia([{ className: 'DocumentAttributeAudio', voice: true }]),
      ),
    ).toBe('voice');
    expect(
      mediaKindOf(
        documentMedia([
          { className: 'DocumentAttributeVideo', roundMessage: true },
        ]),
      ),
    ).toBe('video_note');
    expect(
      mediaKindOf(documentMedia([{ className: 'DocumentAttributeSticker' }])),
    ).toBe('sticker');
    expect(mediaKindOf(documentMedia([]))).toBe('document');
    expect(
      mediaKindOf({ className: 'MessageMediaDice' } as Api.TypeMessageMedia),
    ).toBe('other');
  });
  ...
  it('текст без подписи заменяется описанием вложения', () => {
    ...
    expect(messageText(voice as unknown as Api.Message)).toBe(
      '[Голосовое сообщение]',
    );
    expect(messageText({ message: ' привет ' } as unknown as Api.Message)).toBe(
      'привет',
    );
    expect(
      describeMedia({
