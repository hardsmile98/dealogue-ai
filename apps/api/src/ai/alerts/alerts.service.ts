...
import { toAlertDto } from './alerts.types.js';
import type { AlertDto } from './alerts.types.js';
...
export class AlertsService {
  ...
  /** Все активные алерты чата — в resolved; отдаём чат уже без пометки. */
  async resolveForChat(chatId: string, byUserId: string | null): Promise<TelegramChatEntity | null> {
    ...
    return this.refreshAttention(chatId);
  }

  /** needs_attention на чате = есть хоть один незакрытый алерт. */
  async refreshAttention(chatId: string): Promise<TelegramChatEntity | null> {
    ...
    await this.chats.update(chatId, patch);
