...
- песочница `POST …/ai/sandbox` (`agent/services/sandbox.service.ts`,
  `agent/sandbox/`) — моделирование диалога целиком: шаг за шагом («клиент
  написал», «клиент молчит N минут», «ход вручную», «вернуть боту») на
  виртуальных часах, без отправки и без записи в базу. Состояние диалога
  хранит браузер и присылает его в каждом запросе — сервер между шагами
  ничего не помнит.
...
1. Загрузить библиотеку (`POST …/ai/library/seed` или кнопка в вебе), заполнить
   ...
   «не хватает».
2. Прогнать в песочнице диалог целиком: от первого сообщения клиента до
   напоминаний, промотав тишину между касаниями.
...
| POST | `…/ai/reset` | полный сброс агента на аккаунте: настройки к дефолтам (бот выключается), библиотека, состояния чатов, ходы, черновики, события, алерты и статистика удаляются |
| GET | `/telegram/accounts/:id/ai/overview` · `…/ai/chats` | обзор агента; сводка режим/этап/касание по чатам |
| POST | `/telegram/accounts/:id/ai/sandbox` | песочница: шаг диалога без отправки (состояние ходит в теле) |
