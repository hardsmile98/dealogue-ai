...
`src/bot/core/` (чистые модули с тестами) и `src/bot/services/turn-runner.service.ts`
(оркестрация с базой и моделью):
...
`core/channel.ts`: Telegram и песочница подключаются следующими этапами.
...
5 с; ответ, который не разобрался как JSON, запрашивается ещё раз. Если ход
...
Тот же `TurnRunnerService`, что будет у Telegram, но с другим окружением
(`services/bot-sandbox.service.ts`):
...
`BotSchedulerService.registerChannels` — это делает Telegram-канал при
старте (при выключенном Telegram поллер не запускается). Живая проверка
