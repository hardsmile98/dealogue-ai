...
scheduler.registerChannels({
  environment: (chatId) =>
    chatId === CHAT ? { channel, clock, isStale: () => false } : null,
  isCollecting: () => false,
});

const pendingJobs = async (chatId) =>
  db.query(
    `SELECT kind, run_at, status, payload FROM bot_jobs WHERE chat_id = $1 ORDER BY created_at`,
    [chatId],
  );
...
try {
  ...
  await jobs.createMany(SANDBOX_CHAT, [
    { kind: 'offer', runAt: new Date(Date.now() - 60_000) },
  ]);
  ...
  history.push({
    id: nextId++,
    direction: 'in',
    text: 'Здравствуйте! Хочу бесплатный расклад, код 12',
    mediaKind: null,
    sentAt: new Date(),
    readAt: null,
  });
  await jobs.createMany(CHAT, [
    { kind: 'reply', runAt: new Date(Date.now() - 1000) },
  ]);

  console.log('запущено заданий:', await scheduler.tick());
  await waitIdle();
  console.log('\nагент ответил:');
  for (const m of history.filter((m) => m.direction === 'out'))
    console.log('  →', m.text.slice(0, 140).replace(/\n/g, ' '));
  show(
    'задания боевого чата после хода (ждём «непрочитанное» через 24 ч):',
    await pendingJobs(CHAT),
  );
  show(
    'задания песочницы (поллер не трогает):',
    await pendingJobs(SANDBOX_CHAT),
  );
  ...
  console.log(
    '\nпосле прочтения:',
    step?.kind,
    step?.runAt.toISOString(),
    step?.reason,
  );
  const again = await ladder.reschedule(CHAT, channel);
  console.log(
    'повторный пересчёт — то же время:',
    again?.runAt.getTime() === step?.runAt.getTime(),
  );
  show('задания:', await pendingJobs(CHAT));

  // Ступень созрела, но канал падает — повтор через минуту, а не передача менеджеру.
  await db.query(
    `UPDATE bot_jobs SET run_at = now() - interval '1 minute' WHERE chat_id = $1 AND status = 'pending'`,
    [CHAT],
  );
  failSends = true;
  ...
  await waitIdle();
  show(
    'после сбоя (ждём reply или ту же ступень с retry attempt 1):',
    await pendingJobs(CHAT),
  );
  const state = await states.find(CHAT);
  console.log('режим чата после сбоя:', state.mode, state.handoffReason);
} finally {
  await db.query(`DELETE FROM bot_chat_state WHERE chat_id = ANY($1::uuid[])`, [
    [CHAT, SANDBOX_CHAT],
  ]);
  await app.close();
}
