...
export function DailyTable({ stats, model }: DailyTableProps) {
  ...
  const seriesTotals = model.series.map((series) => ({
    key: series.key,
    total: model.columns.reduce(
      (sum, column) => sum + (column.values[series.key] ?? 0),
      0,
    ),
  }));
          ...
          {[...model.columns].reverse().map((column) => {
            ...
            return (
              <TableRow
                key={column.key}
                hover
                sx={total === 0 ? styles.emptyRow : undefined}
              >
                ...
                <TableCell
                  align="right"
                  sx={[styles.numberCell, styles.totalCell]}
                >
                  {formatNumber(total)}
                </TableCell>
                {model.series.map((series) => {
