 ...
 */
export function useFeedScroll(feedRef: RefObject<HTMLElement | null>): () => void {
  ...
  const remember = useCallback(() => {
    ...
    if (!feed) return
    stickToBottom.current = feed.scrollHeight - feed.scrollTop - feed.clientHeight < STICK_TO_BOTTOM_PX
  ...
  useLayoutEffect(() => {
    ...
    if (stickToBottom.current) {
      ...
      const element = feed.querySelector<HTMLElement>(`[${MESSAGE_ANCHOR_ATTR}="${anchor.current.messageId}"]`)
      if (element) feed.scrollTop += topWithin(feed, element) - anchor.current.top
    }
    remember()
  })
...
function firstVisible(feed: HTMLElement): Anchor | null {
  const messages = feed.querySelectorAll<HTMLElement>(`[${MESSAGE_ANCHOR_ATTR}]`)
