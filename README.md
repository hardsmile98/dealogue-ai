...
Вкладка «ИИ-агент» у аккаунта: ИИ отвечает клиентам от лица менеджера в тех
чатах, где его включили переключателем, дожимает молчащих (1/3/7 дней по
умолчанию) и передаёт клиента человеку, когда тот готов платить — чат
помечается, а в разделе «Требуют внимания» и в Telegram («Избранное» аккаунта)
появляется алерт. Ответ менеджера из Telegram выключает ИИ в этом чате.

Стиль берётся из истории переписок: «Загрузить всю историю» → «Обучить» строит
профиль (как пишет менеджер, фразы, FAQ, возражения, факты, время реакции), а
в каждый ответ подмешиваются похожие прошлые ответы менеджера. Ручной скрипт
продаж в настройках имеет приоритет.
...
Без ключа настройки и обучение доступны, генерация — нет. Для проверки без
внешнего API выберите провайдера `mock` в настройках аккаунта. Подробности —
в [apps/api/README.md](apps/api/README.md#ии-агент).
