...
Сделаны этапы 1–6: фундамент, библиотека, ход агента, касания, работа
менеджера, обучение и стабильность. Бот отвечает на входящие после дебаунса,
ведёт лида до диагностики и дальше по касаниям до напоминаний; в чате видно
режим и этап, что бот понял и что «отправил бы» (в сухом прогоне), ходы можно
оценивать, а плейбуки — проверять в песочнице. Когда нужен человек, в чате
появляется черновик ответа с кнопками «отправить как есть / с правками / свой
ответ / не отвечать», все такие чаты собираются в разделе «Требуют внимания»,
а в Telegram аккаунта приходит короткое уведомление со ссылкой. Бот учится на
практике: в промпт подмешиваются похожие прошлые случаи «клиент → как ответили»,
в библиотеке видно, на какие тексты клиенты отвечают, а редкий отклик помечается
«переписать». За стабильностью следят аномалии (много передач, ошибок,
регенераций) — они приходят алертами. Статистика и приёмка — следующий этап.
